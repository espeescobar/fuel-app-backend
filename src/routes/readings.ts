...
readingsRouter.post(
  ...
  async (req: AuthRequest, res) => {
    ...
    // ACTUALIZACIÓN: Agregamos title y sharedUserIds al validador
    const schema = z.object({
      odometerKm: z.coerce.number().positive(),
      kmPerLiter: z.coerce.number().positive(),
      capturedAt: z.string().optional(),
      title: z.string().optional(),
      sharedUserIds: z.string().optional() // Llega como un string JSON ["id1", "id2"]
    });
    ...
    // ACTUALIZACIÓN: Guardamos el segmento con el nombre y los amigos
    const segment = await prisma.usageSegment.create({
      data: {
        ...
        sharedWith: {
          connect: sharedWithIds.map(id => ({ id })) // Conecta la tabla con los IDs de los amigos
        }
      }
    });
