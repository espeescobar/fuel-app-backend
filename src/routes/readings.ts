...
// --- RUTA 2: GUARDAR VIAJE (CON VALIDACIÓN ESTRICTA) ---
readingsRouter.post(
  "/",
  requireAuth,
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      if (!body.success) return res.status(400).json({ error: "invalid_body", details: body.error.format() });
      ...
      // 1. Buscamos el último registro de ESTE VEHÍCULO (sin importar qué usuario lo hizo)
      const prev = await prisma.readingPhoto.findFirst({
        where: { vehicleId, capturedAt: { lt: capturedAt } },
        orderBy: { capturedAt: "desc" }
      });

      // 🛑 2. BLOQUEO DE SEGURIDAD ANTES DE GUARDAR
      if (prev && odometerKm <= Number(prev.odometerKm)) {
        return res.status(400).json({
          error: "invalid_odometer",
          message: `El kilometraje ingresado (${odometerKm} km) debe ser mayor al último registro de este vehículo (${prev.odometerKm} km).`
        });
      }

      // ✅ 3. SI PASA LA VALIDACIÓN, GUARDAMOS LA LECTURA
      const finalImagePath = req.file ? req.file.path : "NO_PHOTO";

      const reading = await prisma.readingPhoto.create({
        data: {
          userId, // Guardamos quién registró esto
          vehicleId, // Al usar vehicleId, se actualiza para TODOS
      ...
      // Si no hay registro previo, solo guardamos la lectura (primer uso)
      if (!prev) {
        return res.json({ readingId: reading.id, segmentCreated: false });
      }

      // ✅ 4. CREAMOS EL VIAJE (SEGMENTO)
